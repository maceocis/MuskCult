...
//malik adds
//sensor ancillary metadata format. Should be in media-bus-format.h
#define MEDIA_BUS_FMT_SENSOR_DATA 0x7001
#define IMX477_MIPI_SINK		0
...
static int imx477_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_HBLANK:
		ret = 0;//not handled in original source??????????????????
		break;
	default:
		dev_info(&client->dev,
				 ...
				 struct v4l2_subdev_mbus_code_enum *code)
{
	...
	dev_info(dev, "########## imx477_enum_mbus_code ##########\n");
				  ...
				  struct v4l2_subdev_frame_size_enum *fse)
{
	...
	dev_info(dev, "########## imx477_enum_frame_size ##########\n");
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	dev_info(dev, "########## imx477_get_pad_format A ##########\n");
	...
	{
		struct v4l2_mbus_framefmt *try_fmt = v4l2_subdev_get_try_format(&imx477->sd, cfg, fmt->pad);
		dev_info(dev, "########## imx477_get_pad_format  u1 ##########\n");
	...
	{
		dev_info(dev, "########## imx477_get_pad_format  u2 ##########\n");
		if (fmt->pad == IMAGE_PAD) 
		{
			dev_info(dev, "########## imx477_get_pad_format  u21 ##########\n");
		...
		{
			dev_info(dev, "########## imx477_get_pad_format  u22 ##########\n");
				 ...
				 struct v4l2_subdev_format *fmt)
{
	...
	dev_info(dev, "########## imx477_set_pad_format ##########\n");
				...
				struct v4l2_subdev_selection *sel)
{
	...
	dev_info(dev, "########## imx477_get_selection ##########\n");
...
static int imx477_start_streaming(struct imx477 *imx477)
{
	...
	dev_info(dev, "########## imx477_start_streaming ##########\n");
...
static void imx477_stop_streaming(struct imx477 *imx477)
{
	...
	dev_info(dev, "########## imx477_stop_streaming ##########\n");
...
static int imx477_set_stream(struct v4l2_subdev *sd, int enable)
{
	...
	dev_info(dev, "########## imx477_set_stream ##########\n");
...
static int __maybe_unused imx477_suspend(struct device *dev)
{
	...
	dev_info(dev, "########## maybe_unused imx477_suspend ##########\n");
...
static int __maybe_unused imx477_resume(struct device *dev)
{
	...
	dev_info(dev, "########## maybe_unused imx477_resume ##########\n");
...
static int imx477_get_regulators(struct imx477 *imx477)
{
	...
	dev_info(dev, "########## imx477_get_regulators ##########\n");
...
static int imx477_identify_module(struct imx477 *imx477)
{
	...
	dev_info(&client->dev, "$$$ Read I2C: IMX477 CHIP ID = %x $$$\n", val);

	return 0;
}

static int imx477_camera_s_power(struct v4l2_subdev *sd, int on)
{
	struct imx477 *imx477 = to_imx477(sd);
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct device *dev = &client->dev;
...
static int imx477_init_controls(struct imx477 *imx477)
{
	...
	//ret = v4l2_ctrl_new_fwnode_properties(ctrl_hdlr, &imx477_ctrl_ops, &props);
	//if (ret)
		//goto error;
...
static int imx477_check_hwcfg(struct device *dev)
{
	...
	int ret = -EINVAL;
	struct device_node *mipi_ep;
	unsigned int tmp=0;

	mipi_ep = of_graph_get_endpoint_by_regs(dev->of_node,
						IMX477_MIPI_SINK, 0);

	of_property_read_u32(mipi_ep, "sony,t-hs-prep", &tmp);//just debug. Please remove soon??????
	dev_err(dev, "$$$$$ t-hs-prep: %d\n", tmp);
				    ...
				    const struct media_pad *remote, u32 flags)
{
	...
	dev_info(dev, "########## imx477_camera_link_setup ##########\n");

	return 0;
}

static const struct media_entity_operations imx477_entity_ops = {
...
static int imx477_probe(struct i2c_client *client)
{
	...
	dev_info(dev, "########## Probing imx477 Driver V4f ##########\n");
	...
	imx477->sd.entity.ops = &imx477_entity_ops;
	...
	dev_info(dev, "########## End Probing IMX477##########\n");
